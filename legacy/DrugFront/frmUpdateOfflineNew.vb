Public Class frmUpdateOfflineNew
  ...
  Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
    If My.Computer.FileSystem.FileExists(pOffLineFolder & "data.mdb") = False Then
      pMessageBox = New MyMessageBox("ไม่มีไฟล์ฐานข้อมูล Off line ไม่สามารถทำการอัพโหลดได้", "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Stop)
    ...
    For Each mInstance In mProcess
      If mInstance.ProcessName = "DrugFrontOffLine" Then
        pMessageBox = New MyMessageBox("กรุณาปิดโปรแกรม DrugFront Offline ก่อนทำการอัพเดต", "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        pMessageBox.ShowDialog()
    ...
    If mGet(0) = "1" Then
      If mGet(1) <> pBranchCode Then
        pMessageBox = New MyMessageBox("ฐานข้อมูล Offline ไม่ใช่ฐานข้อมูลของสาขานี้ ไม่สามารถอัพเดตได้", "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        pMessageBox.ShowDialog()
    ...
    If mGet(0) = "1" Then
      If CInt(mGet(1)) > 0 Then
        pMessageBox = New MyMessageBox("ท่านต้องทำการอัพโหลดข้อมูลการขาย Offline ไปยัง Server ก่อนทำการอัพเดต", "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        pMessageBox.ShowDialog()
    ...
    pMessageBox = New MyMessageBox("ยืนยันอัพเดตฐานข้อมูล Offline", Me.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question)
    ...
    Try
      ...
      lblComment.Text = "กำลังเตรียมข้อมูล"
      ...
      Application.DoEvents()
      ds = pService.SelectData("Drug", "SELECT GB.*, GI.fixPrice, GI.stickerPrice, UI.unitFactor FROM GoodBarcode GB inner join GoodInfo GI on GI.goodCode = GB.goodCode inner join UnitInfo UI on UI.unitCode = GB.unitCode")
      If IsNothing(ds) = False Then
      ...
      Application.DoEvents()
      ' เฉพาะ ลูกค้าทั่วไปและสมาชิก HUG Club ที่มีการซื้อสินค้าย้อนหลัง 3 ปี
      Dim mDate As Date
      mDate = pService.ServerDateTime
      mDate = mDate.AddYears(-3)
      ds = pService.SelectData("Drug", "Select distinct CI.custCode, CI.custName, CI.custType, CI.custMBPhone from HistSale HS left join CustInfo CI on CI.custCode = HS.custCode where (CI.custType = '1' or CI.custType = '6') and HS.saleDate >= '" & MDYStr(mDate) & "' and HS.branchCode = '" & pBranchCode & "'")
      If IsNothing(ds) = False Then
    ...
    Try
      ...
      mSqlText1(1) = "INSERT INTO SystInfo (branchCode, compName, branchName, branchPrice) VALUES ('" & pBranchCode & "', '" & pCompName & "', '" & pBranchName & "', '" & pBranchPrice & "')"
      ...
      For i As Integer = 0 To dvCustInfo.Count - 1
        With dvCustInfo.Item(i)
          mSqlText2(i) = "INSERT INTO CustInfo (custCode, custName, custType, custMBPhone, flag) VALUES ('" & .Item("custCode").ToString & "', '" & .Item("custName").ToString & "', '" & .Item("custType").ToString & "', '" & .Item("custMBPhone") & "', '1')"
        End With
      ...
      Dim mMembPriceField As String = "price" & pBranchPrice
        ...
        With dvGoodBarcode.Item(i)
          ...
          Select Case pBranchPrice
            Case "1" ' 1->0 เนื่องจากไม่มีราคาระดับ 0 ให้ใข้ราคาระดับ 1 เพิ่มราคาอีก 3% แต่ต้องไม่เกินราคาป้าย (เฉพาะรายการที่ไม่ fixprice)
              ...
              Else
                mGenPrice = .Item("price1")
                mGenPrice = Math.Ceiling(mGenPrice + (mGenPrice * (pPerPrice1ToPrice0 / 100)))
                ' ราคาที่ปรับขึ้นแล้ว ต้องไม่เกินราคาป้าย
                If mStickerPrice > 0 AndAlso mGenPrice / (mGoodAmou * mUnitFactor) > mStickerPrice Then
                  mGenPrice = mStickerPrice * mGoodAmou * mUnitFactor
                End If
              End If
            Case "2" ' 2->1
              mGenPrice = .Item("price1")
          ...
          mSqlText2(i) = "INSERT INTO GoodBarcode (barCode, goodCode, goodAmou, unitCode, membPrice, genPrice) VALUES ('" & .Item("barCode").ToString & "', '" & .Item("goodCode").ToString & "', " & CInt(.Item("goodAmou")) & ", '" & .Item("unitCode").ToString & "', " & CDbl(.Item(mMembPriceField)) & ", " & mGenPrice & ")"
        End With
        pgb1.Value += 1
        pgb2.Value += 1
      ...
      pMessageBox = New MyMessageBox("อัพเดตฐานข้อมูล Offline เรียบร้อย", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
      pMessageBox.ShowDialog()
  ...
  Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel.Click
    pMessageBox = New MyMessageBox("ยืนยันยกเลิกการอัพเดต", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question)
  ...
  Private Sub frm_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    If btnCancel.Visible = True Then
      pMessageBox = New MyMessageBox("กรุณายกเลิกการอัพเดต ก่อนปิดหน้าต่าง", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning)
      pMessageBox.ShowDialog()
